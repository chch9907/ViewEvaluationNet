...
def get_test_list(image_list_path):
    image_path_list = []
    for s_image_name in sorted(os.listdir(image_list_path)):
        s_full_image_path = os.path.join(image_list_path, s_image_name)
        if os.path.isfile(s_full_image_path):
            image_path_list.append(s_full_image_path)
    return image_path_list
...
def get_pdefined_anchors():
    ...
    with open(pdefined_anchor_file, 'rb') as f:
        pdefined_anchors = pickle.load(f, encoding='iso-8859-1')
    return pdefined_anchors